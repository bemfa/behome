...
- **Multiple Login Methods**: WeChat QR code login, OAuth2, and manual private key entry
...
BeHome supports three login methods:

- **WeChat QR code login**: Recommended for most users. No application credentials are required.
- **OAuth2 login**: Requires Home Assistant application credentials.
- **Manual private key entry**: For users who already know their BeHome private key.

### Optional: Setup OAuth2 Application Credentials

Only OAuth2 login requires this step. WeChat QR code login and private key login do not require application credentials.
...
- Set up automatic status polling every 5 seconds
